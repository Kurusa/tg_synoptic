...
class WeatherLess extends BaseCommand
{
        ...
        if ($text) {
            ...
            if ($callback_data) {
                ...
                $this->getBot()->sendMessageWithKeyboard(
                    $this->user->chat_id,
                    $text,
                    new InlineKeyboardMarkup($buttons), $this->update->getMessage()->getMessageId()
                );
            }
        }
    }
    ...
    private function getWeatherMessage(int $city_id, $day = 0)
    {
        ...
        $weatherData = $weatherManager->getWeeklyWeather($city[0]->city_id, Carbon::now()->addDays($day)->startOfDay());

        $template = new \App\Services\WeatherService\Properties\WeeklyService();
        return $template->getTemplate($city[0]->title, $weatherData, Carbon::now()->addDays($day)->startOfDay()->timestamp, Carbon::now()->addDays($day)->endOfDay()->timestamp);
    }
